...
// Algorithm functions (copied from api-server for seed script)
function computeRecoveryScore(skillMatch: number, experienceMatch: number, educationMatch: number, rejectionConfidence: number): number {
  const weights = { skill: 0.4, experience: 0.35, education: 0.15, confidence: 0.1 };
  return Math.round(
    skillMatch * weights.skill +
    experienceMatch * weights.experience +
    educationMatch * weights.education +
    (100 - rejectionConfidence) * weights.confidence
  );
}

function recoveryRecommendation(score: number): string {
  if (score >= 85) return "recover";
  if (score >= 60) return "manual_review";
  return "reject";
}

function computeDecisionConfidence(weights: number[], accuracies: number[]): number {
  const totalWeight = weights.reduce((a, b) => a + b, 0);
  const weightedAccuracy = weights.reduce((sum, w, i) => sum + w * accuracies[i], 0);
  return Math.round(weightedAccuracy / totalWeight);
}

function evidenceStrength(confidence: number): string {
  if (confidence >= 85) return "high";
  if (confidence >= 60) return "medium";
  return "low";
}

function computeGovernanceScore(metrics: {
  ruleAccuracy: number;
  ...
  const weights = {
    ruleAccuracy: 0.2,
    falseRejectionRate: 0.15,
    recoverySuccessRate: 0.15,
    recruiterConsistency: 0.15,
    explainabilityCoverage: 0.1,
    biasRisk: 0.15,
    validationPassRate: 0.1,
  };
  const normalizedBiasRisk = 100 - metrics.biasRisk;
  const normalizedFalseRejection = 100 - metrics.falseRejectionRate;
  
  return Math.round(
    metrics.ruleAccuracy * weights.ruleAccuracy +
    normalizedFalseRejection * weights.falseRejectionRate +
    metrics.recoverySuccessRate * weights.recoverySuccessRate +
    metrics.recruiterConsistency * weights.recruiterConsistency +
    metrics.explainabilityCoverage * weights.explainabilityCoverage +
    normalizedBiasRisk * weights.biasRisk +
    metrics.validationPassRate * weights.validationPassRate
  );
}

function computeATSHealthScore(metrics: {
  ruleHealthScore: number;
  anomalyHealth: number;
  validationCoverage: number;
  alertResolutionRate: number;
}): number {
  const weights = { ruleHealth: 0.35, anomalyHealth: 0.25, validationCoverage: 0.2, alertResolution: 0.2 };
  return Math.round(
    metrics.ruleHealthScore * weights.ruleHealth +
    metrics.anomalyHealth * weights.anomalyHealth +
    metrics.validationCoverage * weights.validationCoverage +
    metrics.alertResolutionRate * weights.alertResolution
  );
}

function computeFinancialImpact(falseRejectionCount: number): { totalCost: number; costPerHire: number } {
  const costPerFalseRejection = 15000;
  const totalCost = falseRejectionCount * costPerFalseRejection;
  const costPerHire = falseRejectionCount > 0 ? totalCost / falseRejectionCount : 0;
  return { totalCost, costPerHire };
}
...
async function main() {
  ...
  for (let i = 0; i < candidateBatch; i++) {
    const role: typeof rolesTable.$inferSelect = pick(roles);
  ...
  for (const rule of rules.slice(0, 8)) {
    for (let run = 0; run < rand(2, 5); run++) {
      ...
      const results = benchmarks.map((b: any, idx: number) => {
